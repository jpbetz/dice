...
export const scenarios = [
  ...
  },
  {
    name: 'settle-magnet',
    tags: ['roll', 'physics', 'cuj8'],
    // THE FLOOR CLAMP KILLS MICRO-BOUNCES, AND THAT WAS NOT ENOUGH.
    //
    // cannon-es has no restitution threshold: it applies the same 0.35 to a
    // 20 unit/s slam and a 0.5 unit/s tap, so the tap bounces forever in
    // miniature. MAGNET.vy is that missing threshold — a die in FLOOR contact
    // moving UPWARD slower than vy has its vertical velocity zeroed.
    //
    // The mechanism is OFF in this build (MAGNET.vy 0) and will stay off. It
    // was measured at vy 1/2/4 on 2026-08-10 (tools/steps/settle-matrix.mjs,
    // 16 paired seeds behind a passing canary) and failed every gate it was
    // built for — including, decisively, its own. Shake went UP (+38% on the
    // soul pool at vy 1), and `hops` — the count of separate times a die goes
    // back up, which is the complaint stated literally — did not fall either
    // (soul 4.88 -> 5.53). Zeroing vy each step does not glue a die down: the
    // contact solver re-supplies the push on the very next step, so the clamp
    // trades one smooth ballistic arc for per-step chatter. It also ADDS
    // replay drift (seed 32676 comes back a 435-frame throw instead of a
    // 450-frame one), where shipped's only movers are pose floats at 5e-6.
    //
    // THIS SCENARIO THEREFORE PINS NO OUTCOME, and that is deliberate: the
    // measured outcome is a refusal, and a test that asserted the refusal
    // would fail the day someone fixes it. It pins the MACHINERY, like
    // `pile-refusal` next door — the sentinel is silent, the clamp is wired
    // into the one stepping path, it touches nothing in flight, and it cannot
    // hover a die. Poses are deliberately not compared for the settled throw:
    // nothing in this engine is byte-identical across a tab's lifetime, so
    // duration and frame count are the observables.
    async fn(ctx) {
      const a = await ctx.newTable({ origin: 'localhost', name: 'Alice', allowSolo: true });
      const pool = ['d8', 'd8', 'd4', 'd6'];
      const seeds = [1000, 8919, 16838, 24757];

      // Every die's height on every frame BEFORE the first frame any die comes
      // near the felt — the part of the film that happens in the air. The
      // clamp gates on floor contact, so this must be untouched no matter how
      // absurd vy is; if the gate were a height band or a bare velocity test
      // instead, this is the check that would catch it.
      const flightY = () => a.eval(`(() => {
        const r = window.__diceDebug.currentRoll;
        let f0 = r.frames;
        for (let f = 0; f < r.frames; f++) {
          if (r.keyframes.some((kf) => kf[f].pos.y < 2)) { f0 = f; break; }
        }
        const out = [];
        for (let f = 0; f < f0; f++) for (const kf of r.keyframes) out.push(kf[f].pos.y);
        return out;
      })()`);

      const throwOne = async (seed) => {
        await a.dbg(`throwSeeded(${JSON.stringify(pool)}, ${seed})`);
        const air = await flightY();
        await a.waitFor('(window.__diceDebug.sim(120), !window.__diceDebug.busy)',
          { desc: `soul/${seed} plays out`, timeout: 30000 });
        const p = await a.dbg('settleProfile()');
        // Read the rest heights BEFORE clearing: the hover check below is the
        // only thing standing between this mechanism and a die frozen in air.
        const maxY = Number(await a.eval(
          'Math.max(...window.__diceDebug.tableDice.map((o) => o.body.position.y))'));
        await a.dbg('clearTable()');
        await a.dbg('sim(60)');
        return { seed, maxY, air, ...p };
      };
      const sig = (p) => `${p.duration}|${p.frames}|${p.nudged}`;
      ...
      for (const s of seeds) off.push(await throwOne(s));
      for (const p of off) {
        assert.equal(p.magnetClamps, 0,
          `seed ${p.seed}: the clamp fired ${p.magnetClamps} times with vy 0 — the sentinel must skip the scan`);
      }

      // --- 2. the clamp is wired into the one stepping path -----------------
      await a.dbg('setMagnet({"vy":2})');
      const on = [];
      for (const s of seeds) on.push(await throwOne(s));
      for (const p of on) {
        assert.ok(p.magnetClamps > 0,
          `seed ${p.seed}: vy 2 clamped nothing — the contact scan is not running inside the bake`);
      }
      assert.ok(seeds.some((_, i) => sig(on[i]) !== sig(off[i])),
        'vy 2 left every throw identical, so claim 3 would prove nothing');

      // --- 3. nothing in flight is touched ----------------------------------
      // The claim that makes this mechanism safe to arm at all: a die that is
      // not touching the felt is never modified, so no trajectory changes
      // before it lands. Compared as floats with a hard tolerance rather than
      // for equality, because the assertion is "untouched", and 1e-9 is nine
      // orders below the 5e-6 the tab moves settled poses by on its own.
      seeds.forEach((s, i) => {
        assert.equal(on[i].air.length, off[i].air.length,
          `seed ${s}: the airborne stretch changed length under the clamp (`
          + `${off[i].air.length} -> ${on[i].air.length} samples)`);
        const worst = off[i].air.reduce((m, y, j) => Math.max(m, Math.abs(y - on[i].air[j])), 0);
        assert.ok(worst < 1e-9,
          `seed ${s}: the clamp moved a die by ${worst.toExponential(2)} while it was in the air — `
          + 'it is not gating on floor contact');
      });

      // --- 4. it cannot hover a die -----------------------------------------
      // The one way this mechanism could be catastrophic rather than merely
      // useless. Gravity adds 110/60 = 1.83 units/s of downward velocity per
      // step, so a clamp that touched DOWNWARD velocity at any vy above 1.83
      // would re-zero a falling die every step and hold it in the air — and a
      // die with zero velocity passes the stillness test and FREEZES there.
      // FLOOR-CONTACT-ONLY is what forbids it — cannon only lists a body
      // against the floor plane when its hull actually penetrates y = 0, so a
      // die in the air is unreachable. Upward-only is NOT the guard: clamping
      // both directions was tried against this scenario and came back green,
      // because a die in contact is already on the felt. Removing the contact
      // gate fails here in 4s with "2 dice ran to SETTLE_CAP at vy 20", which
      // is what a die frozen in mid-air looks like from the outside.
      await a.dbg('setMagnet({"vy":20})');
      const absurd = [];
      for (const s of seeds) absurd.push(await throwOne(s));
      await a.dbg('setMagnet({"vy":0})');
      const ceiling = Math.max(1.3, ...off.map((p) => p.maxY));
      for (const p of absurd) {
        assert.ok(p.maxY <= ceiling,
          `seed ${p.seed}: a die came to rest at y=${p.maxY.toFixed(3)} with vy 20 `
          + `(nothing rests above ${ceiling.toFixed(3)} unclamped) — the clamp is holding dice in the air`);
        assert.equal(p.timedOut, 0,
          `seed ${p.seed}: ${p.timedOut} dice ran to SETTLE_CAP at vy 20`);
      }
      // …and the flight is still untouched at a threshold ten times the
      // per-step gravity increment, which is the strongest form of claim 3.
      seeds.forEach((s, i) => {
        const worst = off[i].air.reduce((m, y, j) => Math.max(m, Math.abs(y - absurd[i].air[j])), 0);
        assert.ok(absurd[i].air.length === off[i].air.length && worst < 1e-9,
          `seed ${s}: vy 20 moved a die by ${worst.toExponential(2)} in the air`);
      });

      // --- 5. the off sentinel restores exactly, leaving no residue ---------
      // vy 0 has to mean "this code was never here", or every measurement
      // taken against it is measuring the instrument.
      const back = [];
      for (const s of seeds) back.push(await throwOne(s));
      seeds.forEach((s, i) => {
        assert.equal(back[i].magnetClamps, 0, `seed ${s}: the clamp still fires after vy 0`);
        assert.equal(sig(back[i]), sig(off[i]),
          `seed ${s}: turning the clamp off did not restore the throw `
          + `(${sig(off[i])} -> ${sig(back[i])})`);
      });
    },
  },
