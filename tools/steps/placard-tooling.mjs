...
export default async function run(stage, [dir = 'tools/out/placard-tooling']) {
  ...
  await t.dbg("tuneSet({'cards.style': 'celtic', 'cards.flourish': 'full'})");
  await t.dbg('sim(30)');
  ...
  for (const [device, width, height] of [['desktop', 1600, 900], ['phone', 390, 844]]) {
    ...
    for (const [style, flourish] of [['stamp', 'full'], ['stamp', 'none'], ['embossed', 'full'], ['parchment', 'full'], ['arcane', 'full'], ['celtic', 'full'], ['celtic', 'rule']]) {
    ...
    if (device === 'desktop') {
      await t.dbg(`tuneSet(${JSON.stringify({ 'cards.style': 'arcane', 'cards.flourish': 'full', 'cards.palette.mode': 'custom', 'cards.palette.text': '#edddff', 'cards.palette.accent': '#ac8ed3', 'cards.palette.surface': '#332548', 'cards.font.family': 'book', 'cards.font.weight': 'regular', 'cards.font.spacing': .08 })})`);
      await t.dbg('sim(30)');
  ...
  const study = await t.eval(`(async () => {
    ...
    rig._ensureBuilt();
    const sheet = document.createElement('canvas'); sheet.width = 1280; sheet.height = 1400;
    const c = sheet.getContext('2d'); c.fillStyle = '#342a23'; c.fillRect(0, 0, 1280, 1400);
    const times = [];
