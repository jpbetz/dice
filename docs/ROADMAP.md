...
| M2c | [**The keyboard path to the dice**](MECHANICS.md) | M1 deliberately did not invent a binding for an action that did nothing; M2b gave it one. There is now a gesture — keep this die — reachable only with a pointer, and *Always interruptible* says keyboard paths exist for the common actions. Small, and it needs a name for the verb more than it needs code. | small | C |
