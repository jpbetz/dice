...
| 14 | [**A place at the table**](BRAINSTORM.md) — BRAINSTORM §1, designed 2026-08-31, **IN FLIGHT: S1–S7 LIVE 2026-09-01, S8 deferred (row 15), S9–S10 owed** | Four people at one table. Just past the near edge, a folded name card in a low holder with your name on it; across the table a second, left and right two more, every card turned toward the reader. Somebody rolls and **the dice come in over their card**, from off the felt, with a soft wash of their hue on the ground under it for as long as they are in the air. You did not read a log line — you know it was Priya because Priya is on the left. That read is [CUJ11](CUJS.md)'s *done when* almost word for word (cited, never minted; CUJ9, CUJ2/CUJ3 and CUJ8 ride along), and CUJ11 is one of the two thinnest journeys in the product. **It supersedes two designs, not one:** [IMMERSION](IMMERSION.md) item 16 "The Table Has Sides" (its two kills — `DecalField` and roster-index seat angles — honoured, its risk clause re-affirmed unamended **in v1, and amended on exactly one of its five clauses by Joe's word in v2 — see the V2 paragraphs at the foot of this row**) and **§12 above**, per-player roll mats, whose named machinery ("a skin over §3's zone machinery") never existed and was superseded by the collect shelf. **The word:** `seat` is the credential and `chair` is the unclaimed pill, both taken, so the new words are **place** (the station, 0–7, server-assigned) and **placard** (the object standing there) — [IDENTITY](IDENTITY.md) §6 decision 7. **The split that makes it lawful:** `player.place` is roster state, display-only, allowed to be wrong; the FILM's inputs are `roll.entry` (0–3) and `roll.lane` (−1/0/+1), stamped server-side in `executeRoll` and riding the roll payload in the seed's own determinism class — so the film never reads the client-mutated roster, and goal 15 is untouched rather than negotiated with. **Stations are absolute** — no rank, no compaction, nobody ever moves — because a placard that shuffles would let a stamped roll name the wrong person, which is the one failure this feature cannot survive. Long edges fill first (heads only at N ≥ 5); a 9th player is placeless with no stamp and no wash, so attribution is edge **plus** wash and an unattributed roll never wears a placed player's name. Per-viewer orientation is quantised to {0, π/2, π, 3π/2} and spends the licence UX §7.55 ruling ② already granted; **the frame outranks the place** — placards are never framing subjects. It takes IMMERSION's Q6 off the board (a six-person felt and a solo felt stop looking identical) and stays inside ruling ④ without spending it. Not beta-gated, no setting; the kill switch is one server constant. **Ten slices, reader before writer, and the entry edge plus the wash is the protected core** — it ships before any geometry exists. The record lands in **UX §7.63** in the ship commit, and this row moves to [SHIPPED.md](SHIPPED.md) in the same one. **Status 2026-09-01:** S1–S7 landed (`1376c9c` … `bb522d6`) with the adversarial review's fixes behind them (`5223639` … `36f4fa6`: the wash keeps the film's clock, a chair that empties on its own is still handed to whoever was waiting, the span probe measures from every chair and the head tax re-priced to −23%), and the tree is deployed. Joe's word mid-run: *"hold on tower interplay till later (future roadmap item), you can stop after S7"* — so **S8 is row 15**, and until it lands a tower table is honestly weaker: three stations stamp side 3 and two stamp side 2 (`js/places.js` TOWER_FLANKS — the wash is the whole of attribution there, pinned in `tests/places.test.mjs`), and a rotated viewer gets the orbit-0 pour acts and then their own frame at the hand-back (`placard-look`'s S8 DEBT PIN). **Still owed on this row:** S9 (the fantasy kits — in moonrise the cards wear the grounded bone-and-brass) and S10 (the phone scenario, UX §7.63, the move to SHIPPED). **V2, THE CARD (2026-09-01).** Joe, on the deployed tree: *"there are some basics that are super broken. The placards or whatever you call them are smaller than the dice"* — measured, a d6 is 1.35 world units on an edge and the card face was 2.00 × 0.70, and on a 1600×900 two-tab shot the far card read 174 × 47 px against a die's 207, with the OWN card sitting under `#result-banner` with the text colliding. Both are the card's own geometry and both are fixed here: the face is **3.00 × 1.80** world (printed band 3.00 × 1.50, ridge 1.83 — bigger than a d6 on every axis), the tent's half-opening drops from 56° to **20°** so the deeper card still fits the frame (holder 1.32 deep, standoff 0.76 vs 1.24/0.72), the atlas grows to 1024 × 2048 for an isotropic 170.7 px per world unit and a 128→88 px fitter, and **the first chair on each long edge moves to an OUTER lane** (`PLACE_LANE` 2.55 → 4.30, stations re-laned so 0/1 are mirrored thirds and the centre slots are dealt last at N ≥ 7) so nobody's own card stands where the banner does. Measured after: far card 233 × 104 px, own card whole in the wide frame, the name clear of the banner by 149–175 px and clear of the widest that panel can grow by 59–69. What it costs, recorded rather than hidden: at `medium` and `close` — where the mat itself overflows the frame — the own card crosses the side rim and the far card crosses the top, so `placard-look`'s rim gates are hard at `wide` and become "the band still meets the picture, the ink still reaches it" at the tighter two. **V2, THE ROOM FOR TWO (2026-09-01, same morning).** The other half of Joe's sentence — *"there is not enough room for two people to roll the dice at the same time... I talked about the latter"* — is BRAINSTORM §1's own *"Their rolls are in a region near them"*, the owner's adjudication of IMMERSION item 16's "owns no region of felt" stated twice; v1 had deferred to the clause and shipped a zero aim, the wrong reading, and **item 16's risk clause is amended by his word** (quote and date beside it) on that one clause and re-affirmed on the rest. Two things changed. **The sweep** (`server.js arrivalSweep`): a placed arrival collects only its roller's own priors and any placeless roll, so **the felt holds one roll PER PLACE**; a placeless roll is the old table (takes the whole felt, and the next placed roll takes it away), a tower table keeps the old rule (row 15), a rethrow collects nothing, a roll whose roller left keeps its felt (the sweep reads the log, not the roster), and history that falls off `LOG_CAP` is CLEARED for the room first so nobody keeps dice the server forgot. `resyncTable` replays every open roll in log order (it was a scalar because the server made it one) and the idle stash is a list. **The landing** (`js/places.js regionFor`/`aimFor`): a long-edge chair owns its near half, split by lane sign into quadrants, a head its end third; the aim box is moved into the region and shrunk to it with a hull cap. Measured, not argued (`tools/steps/place-region.mjs`): the box alone moved where the dice POINTED — spawnDie's target sets direction only, and a die spawned 6–10 units up meets the felt at ~47 u/s and kicks ~2 units wherever its edge points — so the stamped throw is a **low toss**: spawn height 0.45×, hurl 0.5×, into the region's own corner for a laned chair (two rims as backstop), each die at its own rim for heads and the centre slot (a shared box had the pool converging and colliding). Recorded at 24 seeds a cell: the pool's centroid in its region 92–100% for a 3d6 or 6d6 at medium and wide (a lone d20, being its own centroid, 63–92%), dice themselves 85–93% for a 3d6, 63–85% for a 6d6; at `close` a 6d6 no longer fits a quadrant (42–100%). Costs, **recorded not gated** (Joe: "pilling is OK"): the low toss piles 6d6 +5–6pp and 3d20 +8–11pp at medium, 12d6 at a head +19pp, 6d6 at a head at close +22pp; a 3d6 settles 0.2 s sooner, a 6d6 into a corner 0.9–1.5 s later at medium; v1's bars (`place-settle.mjs`, kept as a printout that exits 0) read mean +7.7pp / +0.30 s / worst +22.2pp. Unstamped film bit-identical (every factor 1, every offset +0; `place-seeds-unchanged` green). Pinned by `place-two-rolls` (both pools on both tabs, feltPoses byte-equal, centroids in region with a die of margin, the placeless interplay, the late joiner) and `tests/places-wire.test.mjs §7`; `journey-follow-along` now reads two pools at once. **V2 VERIFIED, THREE DEFECTS FIXED (2026-09-01, afternoon).** The verification pass found what the green suite had not, each by looking or probing rather than by asserting: **(1)** the tower's replay hold (`heldReplay`) was still a scalar while `resyncTable` had started replaying one open roll per place, so a late joiner into a room with a GLB tower socketed and two chairs' dice standing got the last pool only, for good — a Map in arrival order now, drained in log order with the state row consulted first (`place-held-for-tower`, red on a scalar-hold mutation); **(2)** `arrivalSweep` keyed on playerId alone, so a chair handed to a newcomer never swept the departed roller's ghost — the sweep matches the STAMP as well, which makes "one roll per place" literal (§7, two new legs); **(3)** at `medium` the own name was cropped to "ront" by the frame's edge (ink 61 px off the canvas at 1600×900; 310 px at `close`) from both long-edge chairs, and the back chair's own card sat under the bottom rim at every zoom. Two causes, two fixes: **the card's lane is a share of the mat** (`js/places.js placeLane`, a footprint and a gap of pitch at medium — 4.49 / 3.50 / 2.74 — because the presets are similar pictures and both bounds on the card, the frame's edge and the fixed-px banner, scale with the mat; the film's `PLACE_LANE` 4.30 is untouched), and **the camera's half-unit home offset turns with the chair** (`camHomeFor`, and `fitCameraTo` takes its ray in the viewer's own frame — bit-identical at orbit 0), so the back chair now gets the front chair's frame to the digit and `place-two-views` pins the two frames equal. Measured after: own name whole at wide and medium from both chairs (47–60 px inside the canvas at medium, 57–86 px clear of the widest banner, the printed band 13 px clear of it at wide), and at close the first letter loses 7–24 px to the rim. **Costs, recorded:** the full house at close — the seventh chair's card overlaps its neighbours by 0.46 (pinned to the digit; wide and medium keep the 0.30 floor); the heads keep an unshifted target (a shifted one put the far head's card over the top rim); the far card's top 4 px at medium and 66 px at close, and the longest names at close, are unchanged and still recorded. **V3, THE TUNING PASS (2026-09-01, evening).** Joe, on the deployed v2 tree: *"I like a lot of what you have, I think the view should be more zoomed out and the players should have more space between them on the table by at least 20% and the name plaquards should be slightly bigger overall (maybe zoom out 20% and make the plaquards 15% bigger or something)."* Three dials, each one named lever, all landed at their exact numbers (the ±5% by-eye latitude went unspent — the frames said so): **① `PLACE_RETREAT` 1.2** (`js/main.js`) — on a table with ≥ 2 occupied stations every frame `fitCameraTo` fits retreats ×1.2 along its own ray, clamped to the scan's 3.671 ceiling; render-only (ruling ②), and a solo or one-place table multiplies by nothing, which `no-die-sits-in-fog` now pins both ways (front chair camScale 1 solo, exactly 1.2 placed). Dice shrink ×0.833 with it: span 195 → 163 px at medium, 153 → 128 at wide. The fog floor rides the pose destination as ever, so the MULTI-place floors re-pinned (medium 18.68 long edges / 23.75 heads; wide 23.52/30.00; close 14.89 — under the 15.00 tune floor — /18.87) while the solo floors did not move — and re-measuring them found the scenario red on a green tree: its back/head pins (15.93/17.12) predated `camHomeFor` and the head re-price, so the solo table was re-pinned to what the build actually does (16.53/16.53/20.76/20.76, front = back exactly since the half-unit turns with the chair). **② `PLACE_PUSH` 1.2** (`js/places.js`) — every region's centre moves to 1.2× where it stood, walls fixed and open sides pulled in (a quadrant becomes a corner box, the head's end-third an end-fifth), so opposite chairs' felt stands exactly 20% further apart, pinned in the unit table; the throw lane rides the same factor (a lone die at wide comes in at 5.16 = 4.30 × 1.2 exactly; at medium and close the wall room already binds and the laned film is bit-identical — the F1 no-collapse table replays to the bit). Centroid-in-region at the room's own `wide` (24 seeds, raw region, no margin): 3d6 96–100%, 6d6 96–100%, head 3d6 100% / 6d6 83%; medium 3d6 88–96%, 6d6 50–96%; at close a 6d6's centroid no longer fits a pushed corner box (0–8% raw — the small-mat cost, recorded). Settle/pile vs the placeless baseline (24 seeds, `place-settle.mjs`): **mean +8.0pp / median +0.25 s / worst +20.8pp** (close/6d6 at the head) against v2's +7.7/+0.30/+22.2 — the push priced at ≈ +0.3pp mean and the worst cell came DOWN 1.4pp; no cell crossed the +25pp pathological bar, so no cell's push was pulled. **③ the card ×1.15** (`js/placard.js`) — face 3.45 × 2.07 (printed band 3.45 × 1.725, ridge 2.09, footprint 3.68 × 1.52 at standoff 0.86, `TENT_ALPHA` holds at 20°); the atlas takes one more row step AND hands the card region 0.625 of the width (1024 × 2560, 10 MB): **185.5 px per world unit both ways**, still isotropic to the bit, fitter 160 → 108. Net on screen the two camera-facing dials compose as designed: the card is ≈ 0.96× its v2 pixels while the **card:die ratio — the thing Joe judged — improves ×1.15** (medium: far card 299 × 130 px against a die's 220, from 233 × 104 against 207), the far card's printed band now sits wholly INSIDE the medium frame (band ndc ≤ 0.93; it crossed the rim in v2), the own name clears the live banner by 86/69 px (front/back) and the widest the panel can grow by 49/58, and the full house at close fuses the centre slot by 0.568 (from 0.46 — pinned to the digit, wide 1.42 / medium exactly 0.30). Re-taken and judged (`shots-v3/`, 3 zooms × both chairs, desktop + phone): more air around the table, the two pools clearly apart on opposite corners, cards bigger than the dice they name, every name legible — with one ladder note for the record: at `close` with both pools standing the back chair's ladder prefers the dice rung (scale ≈ 2.0), its own lawful call. **V3 VERIFIED, THE PUSH REACHES THE THROW (2026-09-01, night).** The verification pass proved two defects against the tuning above, both fixed. **(D1)** The region centres moved ×1.2 exactly but the FELT did not follow: measured pool separation grew only 6–9% (medium 5.82 → 6.17 for a 3d6) — a third of Joe's "at least 20%". The cause was the AIM, not the containment: the aim box is anchored in the region's WALL corner, which the push never moves, so at `box` 0.5 the corner-anchored centre stood at only ×1.09 of the v2 aim while the centres said ×1.2. The fix is the box dial itself — **`PLACE_AIM.box` 0.5 → 0.25** — which stands the aim centre at ×1.21 of the v2 aim on every mat (pinned in the unit table at ≥ ×1.2 on both axes, every zoom), and the felt moved with it: single-pool separation medium +13% 3d6 / +11% 6d6, wide +17% / +16%; with both pools standing (`tools/steps/place-gap.mjs`, the two-tab flow, 24 seeds) separation medium 6.21 → 6.71 and wide 8.92 → 10.21, the clear ground between the pools medium 3.54 → 4.02 (+14%) and wide 6.14 → 7.11 (+16%). **The remainder of the 20 is a measured ceiling, not an unspent dial:** the box swept 0.35/0.25/0.18 and the softer tosses beside it (h 0.35, speed 0.4, 24 seeds a cell) saturate at 0.25 or give ground back — the hull cap (dice are never aimed at a rim), the wall-bound lane at medium and the settle smear clipping at the rims own what is left. Costs, recorded not hidden: medium containment thins (3d6 centroid 88–96%, 6d6 67–92%, the head 75/50), a lone d20 wanders more at the tighter zooms (42–75%), a close corner box no longer holds a 6d6 centroid at all (0%, from 0–8%), and the settle bars read mean +8.6pp / +0.27 s / worst +20.8pp — the worst cell unchanged from v3, nothing past the +25pp bar, no station's push pulled. **(D2)** `js/places.js`'s dial record and `place-two-rolls`' comment still printed the v2 rates the push had invalidated while this row carried the new ones — the tree contradicted itself; both now carry the v3.1 record and cite it to this row. Shots re-taken (`shots-v3/fix/`): the pools stand on their own corners with clear felt between them, medium two-tab rest centroids (−4.45, 2.55) / (3.95, −2.56). **V4, THE DEMO DOOR (2026-09-01, night).** Joe, after the v3 verification: *"I also think we need a better way to test this out quickly. Maybe a demo mode where you can have a dial to control player count (random names) and some extra lines on the table to show the various regions dividing up the table that are hidden in normal play?"* Shipped as this feature's own TEST SURFACE, not as player chrome: `?demo=1` on a tab with no room (`js/demo.js`). The `?stability=beta` precedent gives the SHAPE — a url param nobody meets by accident — and deliberately not the mechanism: a channel is an ENROLMENT that must outlive a reload, so it is stored, mirrored, redeemed and stripped; a demo is a MODE that lasts exactly as long as its tab, so it is read once, never stored and never stripped, and GOALS §7 is untouched (a dev flag is no more user state than `?lobby=1`). **SOLO-ONLY IS A LAW, NOT A PREFERENCE:** the door stands roster rows no server sent and stamps its own `roll.entry`/`roll.lane` — film inputs of the seed's determinism class — and both are lawful only where there is no second viewer to fork a film away from. So a `?room=` refuses it, loudly, and a demo tab SUPPRESSES THE ROOM MINT; without that it would land online at a table of one and reload into the `?room=` that refuses it. **Four instruments.** ① The DIAL stands 0–8 through the real assignment — the lowest free station by server.js `freePlace`'s own loop rather than by a range, the hue `PALETTE[station]` so a reshuffle moves names and never a tint — into `placeRoster()`, the ONE seam every reader of "who sits where" now goes through, rather than into `players` (which half the chrome reads behind `netOnline && net`, and which would have put fake names in the presence rail, the whisper picker and the pools owner switcher). The names are a fixture in costume — a two-letter one, a 23-character one at the roster's own ceiling, an emoji and a diacritic — because a list of five-letter names proves the card's fitter never runs: measured at eight, `fontPx` 160 → 108 with the long name visibly truncated. ② The SEAT SWITCHER is STICKY, its one difference from `simulatePlaceView`: it writes the dial, `myPlaceRow` derives from the dial, and the eye follows through the shipped `placeOrbitSync` on the shipped roll-boundary flush — so a places flush RESTORES the chair instead of handing a borrowed eye back to station 0, and six chairs are walked in one tab. ③ The OVERLAY is drawn FROM `regionFor`/`aimFor` and from `laneAndLine` (playRoll's own three arguments to `laneSpread`, lifted into one function so the drawn spawn tick and the throw cannot come to disagree) and never from a second copy of their arithmetic — `demo-regions` compares every rectangle the page reports against those functions called in Node against the live extents, and `demo-roll-from-seat` pins the drawn lane against the `laneWorld` a real throw recorded. Region outline + faint hue fill + aim box + spawn line + the station numeral in the region's outer corner, plus the mat's own edge in neutral grey — that last one added by LOOKING: the push leaves a 2.2 × 1.34 corridor of untinted felt down the middle and on a dark felt it read as a gap between two tables. Render-only (`depthWrite` false, renderOrder 9 — over the felt, under the wash at 10) and asserted against `worldBodies()` rather than against a promise in a comment. ④ ROLL FROM SEAT k goes down the production `playRoll` with a lawful LOCAL stamp (`entryFor`, the same function the server calls with the same two arguments), and `soloAutoCollect` gains the per-place sweep (js/demo.js `demoArrivalSweep`, server.js `arrivalSweep`'s three clauses) — so eight pools stand at once where the pre-places whole-felt beat would have left the eighth alone. **The gated claim is the NEGATIVE one.** `demo-door-shut` boots `?demo=0` beside a tab carrying no param at all and compares framing, places, placard budget, world bodies and DRAW COUNT, because "I looked and there was no panel" is exactly the check that stays green while a listener leaks in behind it. **And the look pass is one tab now:** `place-card.mjs` and `place-view.mjs` ride the door (place-view reaches all eight chairs, which three loopback origins never could — the harness ceiling is three tabs), while `place-two-rolls.mjs` deliberately stays two real tabs in one real room, because a stamp crossing the WIRE and two clients baking one film is the one thing a single tab cannot say. Shots re-taken (`shots-v4/`: 2/4/6/8 players, overlay on and off, a seat-3 roll mid-flight and settled, desktop and phone). **THE ROUND TABLE (2026-09-01/02).** Joe, on the deployed v4: *"Please do another pass on the table so that players sit in a circular orientation not around a rectangular table. I'd asked for this originally. 2 players should sit opposite, 3 players should sit in a triangular orientation... it's simple math."* — his own BRAINSTORM §1 sentence (*"Two players get opposing markers; six players get a hexagon"*), which the v1 design had replaced with eight absolute rectangular stations. A first pass (ring-of-chairs around the rectangular mat, a design workflow, four slices) was stopped by him at the picture — *"I hope this is not the actual plan"* — and the second was built by hand from first principles at his insistence (*"WHY DO YOU NEED A WALL?" … "simplify!!!!"*): **① the table is round** — a disc of radius w/2 (`js/places.js ringRadius`); the invisible walls stand at ±w/2 on both axes (the presets are square) and only catch a runaway die; **② seat k of N sits at 2π·k/N** (`placeTheta`; a 300° arc under a tower), the card on that ray just outside the rim (`seatAnchor`), the camera on that ray, and **the dice TOSSED from there onto the spot in front of you** (`seatToss`/`tossAim`: born 0.4 behind a spot at half the radius, height ×0.3, hurl ×0.12, pitch 1.5 — measured with `tools/steps/ring-look.mjs`, every pool's centroid within one die of its spot, 0.1–0.8 world units over three seats). The wire stamp is `roll.seat`/`roll.seats`/`roll.arc` (+`roll.place` for the server's sweep only, which now keys on the CHAIR, so a departed roller's dice do not outlive the chair) — `entry`/`lane` are gone from every writer and reader, and the old algebra stays in the module for the tools that price it. **The table is 2.5× the rectangle** (`TABLE_SCALE`, one dial; the eye presets scale with it) because *"the dice are way too big for that table"*; the camera frames what matters — every seat's spot and the OTHER players' cards, never the empty felt and never your own card, so a lone player sees their pool large and a full table sees the ring, from every chair identically — and the ladder keeps the whole-table frame on a seated table (its dice rungs were turning the table a quarter and zooming onto one pool). Solo is a table for one: the film tosses as seat 0 of 1. The result banner steps aside for a seated viewer (`body.seated`, bottom-left), since your own card now stands where it was. Tests re-pinned for the circle: the unit rows (2 opposite / 3 triangle / 6 hexagon / 4 at the quarter turns, all at one radius; the toss geometry), the wire and demo sweeps, and the scenarios (`place-throws-from-your-seat`, `place-two-rolls`, `demo-regions` and `demo-roll-from-seat` against `seatToss`; the golden stampless film re-recorded once for the new table; `settle-tail`'s cut count RECORDED — on a table this size the hurl parks nothing). **Known red, owed as row 17:** `venue-frame` — the venues' dress was composed on the 11×6.7 rectangle and is out of frame on the round table. | large, 10 slices | B |
| 15 | **Places under a tower** — S8 of #14, **DEFERRED 2026-09-01 by Joe** | The pour-camera interplay #14 stopped short of: the back stations relocate to BOTH flanks live at the tower flush, a rotated viewer's two tower acts are skipped (the `prefers-reduced-motion` shape at `towerCamTo`), the tower rung's resting `orbit: 0` becomes the viewer's `placeOrbit`, and the flank rows get lanes along z so the five stamps a socketed table produces today become eight distinct ones. The anchors have sat in `js/places.js` since S2 (TOWER_FLANKS, unit-pinned) and the server has stamped the flank remap since S5; what is owed is the camera and the film's z-lanes — and the lanes change `spawnDie`'s behaviour under a tower, which is exactly why it waited for its own row. Proof: `place-tower` as SLICES S8 wrote it, plus moving `placard-look`'s DEBT PIN into the `framingInfo().orbit ∈ {placeOrbit, placeOrbit+π/2}` invariant. | med, CRITICAL (camera) | B |
| 16 | **Placards that match dice sets** — Joe's ask (2026-09-01, verbatim: *"Plaquards that match dice sets would be a nice roadmap item"*). **RE-BASED 2026-09-04 by the dresses** (UX §7.65): the table's default is the INLAY now — ink on the felt, no object — so "the card's material kit" is a question about a dress that is no longer the shipped one. What this row means today is **the INK's** identity per house (its hand, its metal, its ornament), and the mechanism it wanted is already standing: `cards.ink.tone` proves an atlas-region repaint per dress at zero new draws and zero new materials, which was the open design question. The tent and the plate keep the material-kit reading, and they are options rather than the table. | The card's MATERIAL KIT keyed to the roller's dice set/house ([THEMES.md](THEMES.md)'s taxonomy — ten houses, seventeen sets): a Classics roller's card wears the grounded bone-and-brass, a Tidewrack roller's the sea-glass, so the card carries per-player IDENTITY the way the dice already do. `entry.set` already rides the roll and `player.set` the roster, so no wire work — this is a placard-dress read of state that exists. **Distinct from #14's S9 on purpose:** the venue dress restyles ALL cards to the room's venue (one kit for the whole table); this row is per-PLAYER — eight different hands at one table. **The design constraint that shapes it:** the placard rig is one merged mesh with ONE material by construction — eight hues ride the vertex colours and the whole rig is ≤ 4 draws (`js/placard.js`; `placard-look`'s budget gate) — so per-player MATERIAL kits fight the 1-draw budget head on. The likely shape is therefore **atlas-region skinning per house** (the row's card/base/bead regions repainted from the house's palette — canvas paint into the row that already repaints on rename, zero new draws, zero new materials; budget cost ≈ atlas repaints on roster/set change only, the recompositeFelt no-churn law as ever), NOT material splits; anything wanting real per-house maps (a mother-of-pearl ORM, say) pays +1 material and more draws per distinct house and must argue with the gate. **Sequenced after S9** — the venue axis must exist first so the two axes compose (the venue picks the kit FAMILY, the house picks the skin within it) instead of fighting. Row only — no build. | med | B |
| 17 | **Venues re-laid for the round table** — owed since the ring (2026-09-02) | The glade, the fae ground and the oak were composed on an 11 × 6.7 felt with the camera on its long edge (`/new-venue`'s fifteen rules, `venue-frame`'s screen-space gates). The table is now a disc 2.5× as wide, framed on the seats' spots and the other players' cards, and the dress stands outside the frame — `venue-frame` and `venue-life` are RED and recorded, not weakened — and so, for the same reason, are `tower-hollowbole` (the pour camera and the tower's place at z0 were tuned over the rectangle; the fall reads 93/99 watchable at wide) and `audio-phases` (the absorbing rule's landing energies were priced on an 11-wide hurl) — the tower and the audio phases are re-tuned here beside the venues. What is owed: re-lay each venue's props round the ring (the rim is the new edge; every chair is the near edge), re-price the fifteen gates from a seated frame at N=1, 2 and 8, and decide whether the dress rotates with the viewer (it is shared scenery, so no — it stands, and the frame turns). Also S9 of #14 (the placards' fantasy kits) rides here. | med, look | B |
| 18 | [**Developer mode, phase 1**](DEVMODE.md) — Joe's brief 2026-09-02, **LANDED** (the loop: 2026-09-02) | `dice.yaml` is the declaration of the app; the backtick is the door; every phase-1 leaf is a dial that moves the live scene through the re-apply functions that already existed; the cast that was `?demo=1` is a section of the panel and `?demo=1` is gone; film dials and the cast lock at a table of two. Landed: the primitives (`js/yaml.js`, `js/tune.js`, the served `js/tunables.js`, `js/devui.js`, `js/devmode.js`), the wiring of fifteen tune objects, the door, `tools/dice-apply.mjs` (Download → the checkout's own `dice.yaml`, line by line) and the proving scenarios (`dev-key-door`, `dev-mode-production`, `dev-export-roundtrip`, `dev-shell-loads` beside the `dev` lane). Phase 2 (the Save route, `table.seats` on both sides, sound/post/cards, clock, bench, A/B, HUD) is DEVMODE §11. Open questions for Joe: DEVMODE §12. | M | A |
...
### 12. Per-player roll mats — CLOSED 2026-08-31, succeeded by #14

Visual skin over §3's zone machinery; mat color per-player, visible to all.

The machinery it named never existed and was superseded by the collect shelf,
so this was closed rather than rewritten — the reasoning is in
[IMMERSION](IMMERSION.md)'s "HOW THIS RELATES TO THE ROADMAP". Its goal is
reached whole by **#14, "A place at the table"**: the player's hue on a real
placard at their station, visible to all, plus a transient wash under it while
their roll is in the air. What it does **not** get is a per-player region of
mat — the felt is still one place a roll lands. The number stays as history
(C4), and #14 took a fresh one.
