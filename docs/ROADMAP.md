...
| 18 | [**Developer mode, phase 1**](DEVMODE.md) — Joe's brief 2026-09-02, **LANDING** | `dice.yaml` is the declaration of the app; the backtick is the door; every phase-1 leaf is a dial that moves the live scene through the re-apply functions that already existed; the cast that was `?demo=1` is a section of the panel and `?demo=1` is gone; film dials and the cast lock at a table of two. Landed: the primitives (`js/yaml.js`, `js/tune.js`, the served `js/tunables.js`, `js/devui.js`, `js/devmode.js`), the wiring of fifteen tune objects, the door and the `dev` scenarios. Left in phase 1: `tools/dice-apply.mjs` and the proving scenarios (`dev-key-door`, `dev-mode-production`, `dev-export-roundtrip`). Open questions for Joe: DEVMODE §12. | M | A |
