...
> **NEXT FREE SECTION NUMBER: §7.44.**
> *(§7.42 and §7.43 claimed 2026-08-15 — the struck die on a per-die surface,
...
| Result banner | §7.11b — the folded card, the hover read, the beacon · **§7.28** — neither the card nor the dice are on a clock; the next roll retires both · §7.21 — the named primary verb, the retired watermark · **§7.24** — `VS DC 15` renders under every system, the adjudication only where a sum does, and the hero slot names the rung (`Face down` / `Whispered`) where the mute gold `?` used to sit · **§7.42** — the outcome rows show EVERY die that was thrown, struck ones included | §7.9's Done-at-rest and its ~6 s clock (the shipped clock is 3 s) · §7.7.2's ⟳ / Collect / ✕ trio · §2.5 (retired 2026-08-08) · §7.24's *Not closed* list, which is now stale in all three bullets — read the struck-through block, not the paragraphs it replaced |
| Verdict card | §7.16's THE FLOW TO COLLECTED — a folded card whose clock shelves the roll · §7.21 — `❯❯ Skip` repaints to `✕ Clear` when the beat lands · **§7.24** — the stake goes into `#verdict-margin`, written ONCE above and outside every branch; the hero holds the reading and the two never share a slot · **§7.42** — the hero's rows show every die thrown, so a `4d6dl1`'s dropped die is now in the rows AND in its `DL1` attribution card | §7.7.2's verdict half (struck there) · §2.4 phase 0's user-held dwell and Roll button (shipped: a 1.35 s timer, no button) · §2.5's one hero slot — retired, struck there. **The card has no subtitle element under any system** (§7.24's deferral); do not read the eight-surface table as though it does |
| Peek | §7.7.1 for what a peek is · §7.11b's "folded card, shelf edition" · §7.21 — its primary is always `✕ Clear`, housekeeping being anyone's · **§7.24** — the stake, and the held word in the total slot (the `?` survives only for `!entry`, and under a totals lens) · **§7.42** — its rows show every die thrown, at `.pk-outcomes` scale | §7.15's one-✕ rule — retired, struck below · §7.9's "the peek carries a prominent ✕ at its base" amendment · §2.5 (retired) |
...
### 7.42 The die a pool discarded, on a per-die surface (2026-08-15)
...
**Scenarios:** `struck-die` — see the list at the end of §7.43.

### 7.43 The ledger sheet — your budget, for tonight (2026-08-15)
...
**Scenarios owed by §7.42 and §7.43** (the hooks are shipped; the scenarios
