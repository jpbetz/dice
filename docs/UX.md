...
| Section bar (`Dice · 2d6 · Your pools`) | **§7.20a, amended 2026-09-11** — ownership is explicit · §7.23, as amended 2026-08-08 — three independent switches, all-off legal, two state objects; the DRESS of record is the comment over `#left-panel :is(.section-seg, #rail-mode)` in `css/style.css` | §7.9's "ONE input view at a time" and the two-value `dice.inputmode.v1` |
...
| Saved-pools rack + shelves | **§7.20a, amended 2026-09-11** — browser storage and ownership note · §7.9's THE SHEET PASS — identity edits by id, creation-as-editing, the ✎ row — and its DEALT RACK amendment · §7.18 for what the ✎ gate covers · **§7.44** — the rack figure is a BUTTON that opens the ledger sheet, and a shelf's target may be the system's or one you typed for tonight | §7.17's region head, amended below: `SAVED POOLS` no longer stands over your own rack · POOL-ANALYSIS §9's "where the rack figure lives" — taken in §7.44; it stays in the head |
...
**Amended 2026-09-11:** a new host joins immediately with an editable guest
name; the unjoined first-time-host exception below is retired. The table now
shows its name (or a short room-derived label) and connection state beside the
roster, with Retry after a failed join. **Your pools** and its browser-storage
note explain ownership across tables; the join screen explains sharing and
names the destination. See [the field report and implementation record](SHIPPED.md#joining-the-table-behind-the-link-2026-09-11).
...
### §7.55 — The camera frames the dice (2026-08-18)

*ROADMAP C27, shipped ON. There is no new control, no setting and no pixel of
chrome in this section: the only thing that changed is where the eye ends up
once the dice have stopped. `FRAMING.preferDice` is `true`.*

#### ① What a player sees

The dice are bigger. On a tablet held in portrait — the device this is really
for — a `1d20` check goes from a die you read to a die you *watch*, and three
d6 fill the middle of the screen instead of sitting in the middle of a mat.

What it costs is the mat's edges. Before this, the camera pulled back until all
four corners of the felt were in the view and then stopped; now, wherever that
succeeded, it asks a second question — *would framing the DICE be better?* —
and takes that frame when the answer is yes. The felt is then **cropped**: its
corners are off screen, and what fills the view is table surface with the dice
on it.

**Joe's call, on the frames rather than on the argument** (2026-08-18,
`shots/v-crop-*.png`, regenerated by `node tools/drive.mjs
tools/steps/verdict-shots.mjs crop`): *"turn preferDice on."* The question it
answered was literally *does a cropped felt still read as a table* — and it
does, because the felt was never the frame's subject; the dice were.

#### ② The two conditions, which are the whole spec

A dice-first frame is taken **only** when both hold:

- **no die leaves the view** (`after.on >= before.on`), and
- **the dice actually get bigger by a margin** — at least `FRAMING.gain`, 1.15×.

Everything else follows from those two lines and nothing else needs saying.
They are why the camera cannot get worse: it returns the old frame's own size
or a bigger one, and never a smaller one. They are also why **big pools are
untouched with no rule about big pools** — a forty-die heap is nearly as wide
as the mat, so framing it can only pull the eye *back*, the margin refuses it,
and the frame stays exactly what it was. There is no die-count threshold in
this feature and there must not be one.

#### ③ The numbers, and the command that reproduces them

`node tools/drive.mjs tools/steps/frame-residual.mjs` — six pools × three
widths × five seeds, both frames read off ONE settled throw so the difference
is the camera and provably nothing else. Median die span in px, 2026-08-18:

| pool | phone 390 | iPad-p 834 | desktop 1600 |
| --- | --- | --- | --- |
| 1d20 | 215 → 239 · fires 2/5 | 119 → **351** · **5/5** | 200 → **246** · 4/5 |
| **3d6** | 73 → 74 · 1/5 | 119 → **199** · **5/5** | 200 → 200 [..253] · 2/5 |
| 6d6 | 62 → 68 · 3/5 | 119 → **159** · 4/5 | 200 → 200 · 0/5 |
| 12d6 | 59 → 63 · 2/5 | 119 → 119 · 1/5 | 200 → 200 · 1/5 |
| 20d6 | 59 → 59 · 0/5 | 119 → 119 · 0/5 | 200 → 200 · 0/5 |
| 40d6 | 59 → 59 · 0/5 | 119 → 119 · 0/5 | 200 → 200 · 0/5 |

Over 90 paired throws: **0 shrank, 0 lost a die, 30 fired.**

**IT IS NOT A PHONE FEATURE, whatever earlier drafts of C27 said.** A 390px
phone gains nothing at the median, and the reason is geometric rather than
photographic: three settled d6 span most of an 11×6.7 mat, so framing the dice
*is* framing the mat at that size. **The phone's lever is the rail** (it takes
112 of 390px before the camera sees anything), and the two do not compose — a
full-width felt makes the mat *fit*, which sends the frame back to the mat rung
and makes dice **smaller**.

#### ④ The table can now turn on a desktop, and that is inside the call

Which way round the table sits has always been decided by measurement rather
than by a viewport threshold (§7 elsewhere; `computeFraming`). Its trigger is
"landscape cannot contain the mat" — and a dice-first frame **gives the mat up
on purpose**, so on a tablet and a desktop the portrait candidate is now
computed where it never used to be, and wins where it frames the same dice
larger.
...
**It is still per-viewer and still invisible to everybody else** (ruling ②):
the MAT does not move, no wire field carries it, and two players at one table
can be looking at the same dice from two different chairs. That is the property
that makes varying by device safe here where it is forbidden for the mat (C24).

#### ⑤ What did NOT change, and the one dial left unspent

- **The deciding die is still never cropped out of frame.** That floor is
  older than this section and this section does not touch it.
- **The eye still never comes closer than the zoom preset.** `FRAMING.floor`
  stays at 1. There is a second, more aggressive version of this feature
  (`{floor: 0.55}`) which lets the dice rung approach; nobody asked for it, the
  2.95× tablet win did not need it, and it is the only C27 dial left unspent.
- **No setting, and there must not be one.** This is not a preference: it is
  the camera doing the job it already had, better, and a toggle would make a
  player responsible for a decision that is a measurement. `__diceDebug
  .setFraming({preferDice: false})` exists for instruments, not for people.
- **Nothing about the roll changed.** The camera reads a settled world; the
  film is a function of the core and the seed, and one seed is still one film
  across every viewport (`one-seed-one-film`).
