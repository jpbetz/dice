...
**A DEFAULT PARAMETER MEASURES AS ZERO-ARG** (2026-09-03, phase D4), and that
is how the rule above was quietly broken for months: `Function.length` counts
the parameters BEFORE the first default, so `mood(on = true)`, `floorLook(opts
= {})`, `tuneReset(scope = 'all')` and `split(name = '')` all read as 0 and the
sweep was calling them — raising a venue, rendering two full frames and reading
back two `toDataURL`s, resetting every dial and MINTING A ROOM — on every run,
staying green because the scenario asserts nothing afterwards. So: **a mutating
hook declares its arguments and puts the default inside the body.** Every
console and tools/steps caller reads the same; the sweep reaches only the
readouts it was written for. A READOUT may keep its default parameter — that
...
### Developer mode — the one-tab look pass, and every dial

The backtick key on any table (or `__diceDebug.devOpen()`) opens
**developer mode** ([DEVMODE.md](DEVMODE.md); `js/devmode.js`, `css/dev.css`,
both loaded only then): a panel over the right edge with every leaf of
`dice.yaml` as a dial, and a **cast** section — the instrument that used to be
`?demo=1` — that stands 0–7 fake players AROUND the viewer's own chair
through the real place assignment (`js/demo.js`), lets you sit in any chair,
draws the ring, each seat's landing SPOT and spawn line straight from the
film's own toss (`js/places.js seatToss`), and throws from any cast chair
with the stamp the server would have written. Dev tooling, not player chrome
— every line of it is behind the door, and `dev-door-shut` is what makes that
a fact rather than an intention: it boots a `?demo=1` tab (now a stray
param, and nothing more) beside a tab with no param, compares their framing,
places, placard budget, world bodies and **draw count**, then opens the door
on one, moves a lamp dial and a gravity dial, deals four, Shuts, and compares
again — and compares the resting poses of one seeded throw on each. "I looked
and there was no panel" is exactly the check that stays green while a
listener leaks in behind it.

**A dev tab is a table of ONE, and the film lock is the law.** A dev tab is
an ordinary tab at an ordinary (minted) table, with a server-assigned place;
the cast is dealt around it and its throws are stamped locally, which is
lawful only where there is no second viewer. So the moment a second real
seat is in the roster the FILM LOCKS (`devInfo().film === 'locked'`): every
changed film-class dial goes back to shipped, the cast is dealt to zero, a
film write is refused by name, and look dials (light, camera, pace) keep
working because they are per-viewer (GOALPOST 7). `dev-room-look` is the
proof: two browsers, one room, the second opens the panel, gravity is
refused, the lamp takes, one roll, both keyframe hashes equal, and the lock
lifts when the other seat leaves. The viewer's OWN rolls at a dev table go
through the server as they always do; only cast rolls are local.
...
back. `tools/steps/place-card.mjs`, `place-view.mjs` and `ring-look.mjs` ride
the door now. `tools/steps/place-two-rolls.mjs` deliberately does **not**,
and stays two real tabs in one real room: that a stamp crosses the WIRE and
two clients bake one film is the one thing a single tab cannot say.
...
node tests/e2e/run.mjs --only dev                  # the door's own scenarios
node tools/dice-apply.mjs ~/Downloads/dice.yaml --check   # the panel's Download, back into the checkout (drop --check to write)
```

**The loop closes through a file, not a route** (phase 1). The panel's
Download is the declaration with your dials in it; `tools/dice-apply.mjs`
validates it against the dial tree (exit 2, one line per problem, nothing
written: an unknown path, a wrong type, an enum value outside its options, any
boolean), works out which leaves DIFFER from the checkout's `dice.yaml`, and
patches the checkout's OWN text line by line — a comment you added locally
after the download survives, and `git diff dice.yaml` is exactly the lines
that moved. Atomic write; `--root DIR` names another checkout. Never run it
against anything but the tree you mean to commit from; the tests never run it
against the checkout at all.

Hooks: `devInfo()` (zero-arg, answers on every tab, production included —
`{panel, film, mode, changed, pending, players, seat}`), `devOpen()` (a
promise: the panel module is imported on demand; null in production),
`devClose()`, `devFold(on)`, `devDeal(n)`; the dial hooks `tuneGet(path?)`,
`tuneSet(patch)`, `tuneDiff()`, `tuneReset(scope)`, `tuneExport()`,
`tuneInfo()` (the writers answer null in production and refuse a film leaf
while the film is locked); and the cast's `demoInfo()` (zero-arg, so P7's
sweep calls it on every ordinary tab too — `seat` is the cast chair you sit
in, or `home`, your own), `demoDeal(n)`, `demoSit(k)`, `demoRoll(k,
notation)` (from your own chair it is your own roll, through the server, and
says `via: 'server'`), `demoRollAll()`, `demoRegions(state)` — one enum of four,
`'disabled' | 'regions' | 'framing' | 'all'` (the first door's `'enabled'`
and `true` still mean the regions layer; a word that is not a state answers
`null` and changes nothing) — each of the mutating ones returning `null`
rather than pretending on a tab without the door. `demoInfo().overlay` is
the overlay's own report: `state`, `objects`, `bodies` (always 0),
`stations` (the regions layer's marks) and `framing` (the framing layer's
hull, discs, cards, lamp cone and walls, or null).

The **bench** (phase 2, 2026-09-02) rides the same rule. Readouts:
`devHud()` (`{fps, calls, tris, bodies, settle, frames, seed}` — fps off the
rAF ring, which fills only while the panel is open, so a scenario that wants a
number presses `devOpen()` first; `calls`/`tris` are `renderAudit`'s own
counters and `settle` is the last film's BAKED duration, not a stopwatch),
`devClockInfo()` (`{state, live, frame, frames, time}`), `devBenchInfo()`,
`devAbInfo()`. Instruments: `devClock('running'|'frozen')` (the panel's own
freeze — `holdClock` is still the scenarios', and Shut releases only a freeze
the panel asked for), `devStep()` (exactly one baked frame, off the tempo
curve), `devScrub(frame)` (moves the projector to a keyframe; walks the impact
cursor past what it skipped without voicing it, so a scrubbed passage is
silent), `devBench(seed, notation?)` (a seeded LOCAL throw — the seed feeds
`composeRoll` at one end and the bake at the other, so the same seed is the
same faces AND the same poses; the row is labelled `bench`), `devReplay()`,
`devHold('a'|'b')`, `devSlot('a'|'b')`, `devFlip()` (swaps the live slot, and
replays the last seed only when the two slots differ on a film-class path).
...
`devFeltApply(id)` (wears it on THIS tab — never the wire: a felt only this
checkout declares is one nobody else could resolve, so Save + reload is how it
becomes the table's felt). The three writers answer a REPORT `{id, refused,
felts}` rather than null for a refusal they can name — a shipped id, a dotted
id, a field that is not a field — because null is the gate's answer and
nothing else's.

The **sets editor** (phase D2, 2026-09-03) is the second, over the dice
catalogue that moved into `houses:` in dice.yaml. Readout: `devSets()` —
zero-arg, answers on every tab, with `houses` and `sets` (the whole catalogue,
each row carrying `inFile`, `removable` and `current`), `current` (the set THIS
viewer wears), `venue` (the set a fantasy venue is staging over it, which
outranks `current`) and `standing` — what the dice on the FELT are made of,
read off the meshes rather than off the recipe, which is how a live reskin is
falsifiable at all (`edge` is the chamfer band's colour, the one number that is
a function of the recipe and a property of the object). `devSetRecipe(id)` is
the row as the panel's form reads it. Writers, all taking an argument, all
answering `null` with the door shut or in production: `devSetClone(id,
newId?)` (a copy into the `house` house at `<set>-2`), `devSetSet(id, patch)`
(dotted sub-paths — `body`, `geo.bevel`, `sound.weight` — through `tune.set`,
so they land in the diff and in Save), `devSetRemove(id)` (a row this editor
made; a shipped one is refused by name), and two that answer `null` while the
film is locked as well, because both put an invented set in front of another
viewer: `devSetApply(id)` (the ordinary `setDiceSet` path) and
`devSetBench(id, seed?)` (one of every die type, down the bench path).
`devSetApply` has one refusal that is not a gate and does not answer null: a
row dice.yaml does not declare yet comes back `{ set, refused: [[path,
'unsaved']], sets }` and the viewer keeps the set they had — a dice set rides
every roll to the server, which resolves it out of the file, so wearing an
unsaved row 400s the roll button (`unknown_set`) with only a page banner to
say so. A Save through the armed route lifts it in the same tab: the server
re-reads the file it just wrote, and `inFile` follows. The
repaint of the standing dice rides a 140 ms timer; every one of these hooks
flushes it first, so what they answer with is the table after the repaint, and
a bare `tuneSet` on a `houses.*` path waits it out like a dragged slider does.

The **recorder and the pop-out** (phase D5, 2026-09-03) close the loop out of
the tab. Readouts: `devRecording()` (zero-arg, `{state, ops, full}` — every
`tune.set` patch, cast deal, seeded throw and asset-row clone written down
while it is armed) and `devSnapshot()` (what the second window is handed:
`table` — the id of the tab that sent it — `T`, the diff, the changes, the
reload-pending list, the film and the venue). Writers, each taking an argument
so P7's zero-arg sweep neither presses Record nor opens a window:
`devRecord('start'|'stop')` (a word that is not a state answers null
and changes nothing, as `demoRegions` does), `devStepText(name)` (the
`tools/steps` skeleton the ops would emit — `js/devmode.js emitStep`, so it
answers null until the door has imported the panel), `devPopout()` (opens
`dev.html` in its own window, at the size `devInfo().popout` publishes) and
`devRowAdd(where, id, row)` / `devRowRemove(where, id)` — `tune.addRow`'s own
signature, and the door a recorded step replays a CLONE through, since
`tuneSet` refuses a field of a row that is not there. The recorder is a
LISTENER on `tune.watch`, so a console `tuneSet` is in the step exactly as a
slider drag is, and a Clone is in it as the row it minted; a field it still
cannot carry comes out as a `// note ·` line in the emitted file rather than
silently. The step goes out through the ordinary blob download and never
through `/api/dev/write`, because a step is code. Shut clears the reel, closes
the pop-out's channel and stops the recording — a Shut is "this tab never
opened the door".

The harness opens one with `ctx.devTab({ players })`:
`newTable`, then `devOpen()`, then `devDeal(players)`. A cast of four beside
a viewer at place 0 stands at 1..4; a full house is the viewer plus seven.
A **phone** one is `tableTab(ctx, { device: { w: 390, h: 844 }, coarse: true })`
followed by `devOpen()`: the metrics and the pointer have to be set before the
first navigation, and the panel reads `(pointer: coarse)` at mount — headless
at rest is `pointer: none`, so the sheet is off unless a scenario asks for it.
...
Tagging those with a journey would claim they prove one.
...
| `log`      | The roll log as a record and as an ARTEFACT. `log-export` asserts on `__diceDebug.logExport.text` — the bytes both verbs write — never on the screen, because the claim is about the file: every roll is a line, the file runs oldest-first where the screen runs newest-first, a face-down roll exports as die TYPES plus the word (asserted as a whole-line SHAPE, since an absence check on a two-digit total collides with the clock), the find box does not narrow it, and the verbs arm on the INCREMENTAL append path — a scenario that collects a roll first passes against the broken code, because collecting forces a full render. Its truncation half is a tail on `endurance-log` (the one scenario already paying for a long log): **`a.roll()` cannot pass `LOG_CAP`**, since it waits for `#log-list` to GROW and the DOM stops at 100, so past the cap the wait is on `currentRoll.rollId` changing |
| `perf`     | Cost and endurance: determinism under the SAP broadphase (`perf-determinism`), the append+prune log at 108 rolls past a 100-cap (`endurance-log`), and the FRAME'S DRAW COUNT (`scene-draw-budget`, ROADMAP V4) — one plain frame and one forced-post frame with the heaviest tower up. Its floor (`calls > 40` on the POST frame) is the anti-collapse guard and belongs on that frame alone: three.js resets `renderer.info` inside every `render()` and js/post.js issues 8 a frame, so a naive read reports the closing quad's **1** draw call where the frame drew 246. The same floor on a plain frame reads 81 when sabotaged, which is above any sane value and can never fail — **a budget with only a ceiling cannot fail either, so sabotage-check the SPECIFIC assertion, not "something"** |
| `visibility` | The ladder (goal 11): held · secret · whisper, reveal authority, offers/dice tower, redaction on the raw wire |
| `chrome`   | The persistent rail, the ONE Pools panel (`dice.panels.v1`, legacy two-region state migrates; emergent compact view), the identity chip (rename / leave & switch), and the *pool / saved pool* vocabulary — `terminology` scans every readable label, tooltip and placeholder across the standing chrome AND the result surfaces (banner, ceremony layer, peek, log flyout, offers, offer menu, seat modal, status pill), for `tray`/`group`/`rack`/`compose`/`category`. **It plays the table first, and asserts every swept root CARRIED TEXT before judging a word**: four of those eight surfaces hold zero characters at rest, so sweeping a quiet table is a green check over nothing. Also the pinned log vs the result banner (`flyout-banner`), which measures OCCLUSION with `elementFromPoint` rather than trusting `--banner-lift` — and whose rotation leg **was a known RED and is now green**: `syncFlyoutLift` was driven only by a ResizeObserver on the two boxes, and a rotation can move them into one column without resizing EITHER — so the observer never fired, the lift stayed stale, and the log covered 251/900 of the read until the next roll (measured 2026-08-17, 1440x900 → 844x390; closing and re-opening the log cleared it, which is what proved the geometry right and the trigger missing). Fixed by also syncing on `resize`, the same event the felt re-fits on. The assertion was never weakened to reach green. **And a scenario in this tag DECLARES the panel state it judges** (2026-08-18): `dice.panels.v1` is per-ORIGIN localStorage that outlives a room, so `hidden-means-hidden` and `a11y-modals` were judging whatever the previous forty `localhost` scenarios had left — both failed the sweep at 219/222 and passed 1/1 alone, because `framing-keeps-the-deciding-die` had gone red on a stale claim and SKIPPED the restore sitting at the end of its body. A cleanup only reached when nothing went wrong is not a cleanup: it belongs in a `finally`, on the scenario's own origin (`flyout-banner` and `framing-prefers-dice` were already written that way). Both victims now set the state instead of inheriting it, which promptly exposed what the inherited state had been hiding — `.hidden` on `#rail-pools` was a NO-OP under a collapsed panel, green until then only because the expanded panel hides that element by a different rule entirely (UX §7.58) |
...
| `table-file` / `prepared-seat` | The prepared table (Tier G) as amended by §11: a chosen seat now becomes a PROFILE of the player's own rather than merging into their one rack, so `prepared-seat` asserts that their own profile is left untouched. Also C22's table half — `setup-stamp` proves the whole stamp chain in ONE assertion (`roomStamp === stamp`: writer → net.pushTable → server → broadcast → judged read, null if any link stops carrying it) plus the refusal being reachable and not spoken over by "X prepared the table", and `setup-stamp-replay` the two legs a push cannot reach (an absent stamp stays absent; a NEWER-major record is not replayed at all). The tower travelling in the file is `prepared-tower`, whose load-bearing pair is `tower === 'bastion'` **and** `towerBodies().length > 0` — socketed with colliders in the world, not merely selected in a menu — proved again for a JOINER. And `peek-presence` is what the pre-join peek says about who is SITTING there: C17 gave it a second seat source (every seated player's published library, each seat carrying `from`, their DISPLAY NAME) and no assertion covered presence at all, so a player who has published nothing sits at the table and the peek must not mention them |
| `groups`   | Saved pools: inline row editor and popover update write back by id, save-as-variant stays additive (the tag keeps the `groups` spelling, like the code). With `meanings`, the two pre-roll forecast surfaces: `pool-forecast` is the per-die spectrum under soul-deal and `sum-read` the distribution of the TOTAL under dnd (§2l ⑥, POOL-ANALYSIS §10). All of `sum-read`'s failure modes live BETWEEN the arithmetic and the paint, which is why its every assertion reads the RENDERED popover — a cell drawn at its index instead of its total, a tie-break sold as a peak, a refusal printing `0%`, and the dashed target painting exactly onto the solid average so two facts read as one mark (`dcAt` was the RIGHT NUMBER throughout, so only the rendered x can catch it) |
...
| `dev`      | DEVELOPER MODE ([DEVMODE.md](DEVMODE.md); `js/tune.js`, `js/devmode.js`, the cast in `js/demo.js`). The scenario that matters most is the NEGATIVE one: `dev-door-shut` compares a `?demo=1` tab (a stray param now) against a tab with no param at all, down to the draw count, then opens, dials, deals and Shuts on one and compares again, poses of a seeded throw included — because the load-bearing claim about a dev instrument is that nobody who did not ask for it ever meets it, and that Shut means shut. Then `dev-dial` (the ladder the cast deals AROUND the viewer's own chair is the ladder a real table deals, and the seat switcher survives a places flush where `simulatePlaceView` could not), `dev-room-look` (the film LOCKS at a table of two: gravity refused, the lamp taken, one roll and two equal keyframe hashes, the lock lifting when the seat leaves), `dev-regions` (every spot and spawn line the overlay reports compared against `seatToss` called HERE — an overlay that redrew the arithmetic would be a confident picture of a mechanism that had since moved), `dev-framing-overlay` (the same standard held against the SECOND layer, 2026-09-02: the drawn hull equal to `fitProbe()`'s own point set in its own order, the card rectangles equal to `placardFootprint` computed in Node, the wall lines equal to `wallPositions()`, a lamp dial moving the cone with nobody asking for a rebuild, `all` counting the framing five plus the regions layer's ring and three marks a chair, and — the two claims the layer's licence rests on — not one body added and, after Shut, the same draw count the tab had before the door), `dev-roll-from-seat` (the local stamp going down the production `playRoll`, ranked with the viewer's own chair; the per-place sweep leaving four cast pools and the viewer's own standing), `tune-wired` (the declaration feeds the scene: a dial moves the SpotLight and cannon's gravity, and the export is the file byte for byte), and the loop's own four (2026-09-02): `dev-key-door` (REAL key events over CDP, matched on `e.code`, walking shut → open → folded → open, inert while typing in the roll box, and the URL and localStorage snapshotted before the first press and equal after Shut), `dev-mode-production` (a SECOND server for the scenario's length under `DICE_MODE=production`: `devOpen()` null, the key inert, the cheat-sheet row hidden, every writer null and every reader answering — beside a development tab where the same row shows), `dev-export-roundtrip` (lamp height and table scale set on a dev tab; the lamp now, the mat once quiet; `tuneExport()` equal BYTE FOR BYTE to `patchYaml` of the checked-in file computed in Node; `tools/dice-apply.mjs` reporting the two changes against a scratch copy of the tree, writing nothing under `--check` and the patched checkout without it; Reset making the export the file again) and `dev-shell-loads` (`tools/devshell.html` with zero exceptions, one row per leaf of the dial tree, and NO engine module on the network log — the `look` tag stays off it because the lane's guard is main.js's own dice counter and the shell has no main.js; the network log is the cosmetic claim instead). Phase 2's bench adds four more (2026-09-02): `dev-hud` (every number in the footer against an independent witness — `renderAudit` for calls and triangles, `worldBodies` for bodies, the baked film's own duration for settle — and fps still reporting under a FROZEN projector, because "how fast is this tab drawing" is a question about the machine), `dev-clock` (freeze holding the felt across half a second of WALL time with a film in flight, a step advancing exactly one keyframe, a scrub moving the projector and the next step continuing from it, and the corner glyph reading `frozen` when a frozen table is folded away), `dev-bench` (the same seed twice giving the same poses and the same faces, a different seed giving neither, the log row saying `bench`, and — the load-bearing one — a SECOND browser at the same room whose log is empty, because a bench throw that reached the wire would be a bench that lied about its seed) and `dev-ab-same-seed` (two held patches: differing on `light.lamp.y` the flip moves the lamp and not one die, differing on `throw.physics.floor.friction` it replays seed 777 and the poses differ). That last one uses friction 0.05 rather than 0.95 for a measured reason recorded beside it: a floor friction ABOVE the shipped 0.6 bakes an identical film. And `dev-sound-post` (2026-09-03) takes the three small sections in the three shapes they actually have: `sound.master` read off the master GainNode's own `gain.value` — including the leg that catches a second writer, since mute and unmute must come back to the DIAL and not to js/voices.js's shipped 0.7 — `sound.impact.gain` through `impactPresetFor`/`impactVoicingFor` so the resolver answers rather than the tree, `post.bloom.threshold` off the `uThresh` UNIFORM, and `cards.*` as the declaration's own round trip: the dial moves the live ring, Save writes the three leaves into a scratch checkout through the armed route, and a fresh tab there boots with the ring where the FILE says and `placardBudget().base` the declared footprint — which is the only way to catch a const captured at module load, the trap js/placard.js's `baseW()` exists to avoid. (That leg used to assert the opposite, and said so: a ⟳ row must not move the live ring. It is an apply row since phase D4 and the assertion moved with the design.) And `dev-absent-in-prod` (2026-09-03) proves the OTHER production answer, the one that does not depend on a value in a file: a server from a scratch tree with `js/devmode.js`, `js/devui.js`, `css/dev.css` and `dev.html` deleted — exactly what `.gcloudignore` withholds from the deploy, the pop-out page pinned here because static-cache.test asserts it IS served locally and a re-added upload line would otherwise be invisible — and `DICE_MODE` unset, so the tab believes it is in development and the panel is simply not there. It boots with zero exceptions and zero console errors, `devOpen()` answers null, the real backtick key leaves `devInfo().panel === 'shut'` and adds no `#dev-panel` to the document, and `tuneSet` still moves the lamp — because the declaration is what the app is configured by and the panel is only a way to turn its knobs. Phase D4 adds two (2026-09-03): `dev-presets` (the door as the gate, a preset of NOTHING refused by name, Hold and Apply read off the lamp's own SpotLight and cannon's own gravity, the row in `tuneExport()` under `presets:` and out again, and a second seat where the look row lands while the film row comes back refused — a preset is applied like a paste, so it is refused where a paste is) and `dev-cards-live` (also tagged `place`: the ring and the rig's own baked pad moving at the flush, a dial turned with dice in the air changing nothing until they land — read in ONE eval under a held clock, so the claim is about the gate and not about a race — and the `cardClear` law refusing a typed card deep enough to reach past the rim). Phase D5 adds the last three (2026-09-03): `dev-phone-sheet` (390×844 with a COARSE pointer, which is the half of the query a width cannot fake: the sheet on the bottom edge at 45dvh and no wider than the screen, the panel starting FOLDED with the corner glyph as its one control, a REAL tap unfolding it — `el.click()` proves nothing about a control a thumb has to reach — every visible row over the 44px floor, the lamp row drawn as a stepper with no `input[type=range]` anywhere in the sheet, a tap on its + moving the leaf by the dial's own step AND the SpotLight with it, and the canvas the same rectangle before the door and after the sheet is up, because the panel is an overlay and not a rail), `dev-recorder` (the door as the gate; three ops in the order they happened with the throw's SEED kept; the emitted step carrying them in that order and PARSING as JavaScript, checked by `node --check` on the bytes themselves; a sets-editing session — Clone, then a recipe edit — coming out as `devRowAdd` of the house, `devRowAdd` of the set and `tuneSet` of the field, so a step replays what was authored rather than noting that it could not; a field of a row the reel does NOT build coming out as a `// note ·` line naming it, and a reel of nothing but notes emitting them rather than the empty skeleton; the tab's own fetches watched to prove nothing was posted to `/api/dev/write`, because a step is code and the route is not for code; and Shut clearing the reel) and `dev-popout` (two pages on one origin: `dev.html` mounts the panel over a MIRROR, at the size `devPopout` itself publishes so the shipped configuration is the tested one; a `set` from the second window moves the TABLE's own SpotLight, a refusal comes back by name from the tune that owns the tree, a dial turned at the table reaches the pop-out's snapshot, and closing either side leaves the other one working — the door shut first, then the window. Three more since the D5 review: it is a WINDOW and not a phone — no sheet dress, sliders not steppers, the `file` section on screen and the section bar not a hidden side-scroller, held again at 390×844 with a coarse pointer; a SECOND table tab on the same origin is not steered by it — one write moves the latched table's lamp and not the other's, and eight samples across two heartbeats show the mirror holding one tree rather than flickering between two; and after the table's door shuts the mirror REFUSES the write by name instead of holding a value nobody is holding). The apply tool's own unit suite is `tests/dice-apply.test.mjs`, every run against a scratch copy of the tree |
